...
class ProgramController < ApplicationController
  ...
  def show_program
    ...
    agency_related = create_graph_nodes(@program,'public_agency_id',PublicAgency,1)
    last_id = agency_related[0].last['id']
    
    company_related = create_graph_nodes(@program,'company_id',Company,last_id)
  ...
  def create_graph_nodes(program, field_entity, class_entity, id_graph)
    ...
    begin
      program_agency = create_data_program(program.id,field_entity,class_entity)
  ...
  def create_data_program(program_id,field_entity,class_entity)
    select_distinct = Expense.select('DISTINCT('+field_entity+')').where(program_id: program_id)
    name_value = []
